...
export function MoveCounter() {
  ...
  return (
    <div className="flex flex-col justify-center rounded-2xl border border-slate-200/10 bg-slate-950/35 px-4 py-3 text-right shadow-lg shadow-slate-950/20" data-aids-used={usedAids ? 'true' : 'false'}>
      <div className="flex items-center justify-end gap-2">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-slate-400">{t('label')}:</p>
        <p className="font-display -translate-y-[2px] text-3xl font-black leading-none text-cyan-100">{moves}</p>
      </div>

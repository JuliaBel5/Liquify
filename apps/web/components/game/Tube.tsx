...
export function Tube({ index, tube, selected, hinted, disabled, onSelect }: TubeProps) {
  return (
    <button
      ...
      className={`group relative flex h-44 w-12 items-end justify-center rounded-b-[2rem] rounded-t-md border-2 border-slate-200/45 bg-slate-950/30 p-1 shadow-tube transition-colors duration-200 hover:border-cyan-100/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200 disabled:cursor-wait sm:h-64 sm:w-16 sm:rounded-b-[2.35rem] ${selected ? 'border-cyan-100 ring-4 ring-cyan-200/35' : ''} ${hinted ? 'hint-highlight border-lime-100' : ''}`}
    >
      ...
      <span className="relative flex w-full flex-col-reverse items-center gap-1 overflow-hidden rounded-b-[1.45rem] pb-1 sm:rounded-b-[1.8rem]">
        {tube.map((color, layerIndex) => (
          <span
            ...
            data-color={color}
            className={`h-10 w-10 rounded-md border border-white/25 shadow-inner transition-all duration-300 sm:h-14 sm:w-14 ${liquidClasses(color)}`}
          />
        ))}
      </span>
    </button>
